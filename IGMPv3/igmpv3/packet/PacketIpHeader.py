...
class PacketIpHeader:
    ...
    def parse_bytes(data: bytes):
        (verhlen, ) = struct.unpack(
            PacketIpHeader.IP_HDR, data[:PacketIpHeader.IP_HDR_LEN])
        ver = (verhlen & 0xF0) >> 4
        print("ver:", ver)
...
class PacketIpv4Header(PacketIpHeader):
    ...
    def parse_bytes(data: bytes):
        (verhlen, tos, iplen, ipid, frag, ttl, proto, cksum, src, dst) = \
            struct.unpack(PacketIpv4Header.IP_HDR,
                          data[:PacketIpv4Header.IP_HDR_LEN])
...
class PacketIpv6Header(PacketIpHeader):
    ...
    def parse_bytes(data: bytes):
        (ver_tc_fl, _, next_header, hop_limit, src, dst) = \
            struct.unpack(PacketIpv6Header.IP6_HDR,
                          data[:PacketIpv6Header.IP6_HDR_LEN])
